...
def ask_for_player_move():
    """Let teh player select a color to paint upper left tile"""
    while True:
        bext.fg('white')
...
while True:
    ...
    if has_won(new_board):
        display_board(new_board)
        print(f"You have Won! {smiley}")
        break
    elif moves_left == 0:
        display_board(new_board)
        print("You have run out of moves. Better luck next time!")
        break
